...
ready(function(){
    ...
    download.onclick = function(){
        ...
        //luohe
        OutPutFileName = fileinput.files[0].name.substring(0,fileinput.files[0].name.indexOf("."));
        var transform = {};
        var now = new Date();
        transform = "HintSetVersion: 1.0\r\n";
        transform += "HintSetName: HintSet_基本款\r\n";
        transform += "Author: null\r\n";
        transform += "Hintset ID: HID基本款\r\n";
        transform += "Creation Date: 07-08-2018"+"\r\n";
        transform += "Hintset Fingerprint: 326.216,355.501575\r\n";
        for(var i=0; i<bins.children.length; i++){
            ...
            for(var j=0; j<b.children.length; j++){
                //luohe add
                if(b.children[j].tagName == "g") {
                    var g = b.children[j];
                    var gChild = g.children[0];
                    b.children[j].setAttribute("pieceId",gChild.getAttribute("id"));
                    b.children[j].setAttribute("id","a"+j);//设置g元素的id
                    var rect = document.getElementById("a"+j).getBBox();//获取path的宽度和高度
                    var tran = g.getAttribute("transform").replace(/translate/,"").replace(/rotate/,"").replace(/\(/g,"").replace(/\)/g,"");
                    var trans = tran.split(" ");
                    transform += "pieceName: "+gChild.getAttribute("id")+"\r\n";
                    transform += "hintedOriginX: "+trans[0]+"\r\n";
                    transform += "hintedOriginY: "+trans[1]+"\r\n";
                    transform += "hintedBoundaryWidth: "+rect.width+"\r\n";
                    transform += "hintedBoundaryHeight: "+rect.height+"\r\n";
                    transform += "hintedRotationDegree: "+trans[2]+"\r\n";
                }
        ...
        //luohe add
        var hintOut = new Blob([transform], {type: "text;charset=utf-8"});
        saveAs(hintOut, OutPutFileName+".hint");
        var output;
